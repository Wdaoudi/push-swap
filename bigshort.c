...
/*   Updated: 2024/09/14 16:18:13 by wdaoudi-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void higher_half_to_b(t_stacks *both,int size,int *tab)
{
    ...
    {
        if (stack->head->content > tab[size*(3/4)])
        {
            push_b(both->a,both->b);
            if(stack->head->content <= number_input/2)//comment trouver la valeur
                rr(both->a, both->b);
            else
                rotate_b(both->b);
        }
...
void lowest_half_to_b(t_stacks *both, int size, int *tab)
{
    ...
    stack= both->a;
    number_input = size;
    while (find_top(stack)->content > tab[size/4] && ft_lstsize(stack->head) > 3)
    {
